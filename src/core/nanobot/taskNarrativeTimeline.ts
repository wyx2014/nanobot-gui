...
export function buildTaskNarrativeEntries(messages: Message[]): TaskNarrativeEntry[] {
  ...
  messages.forEach((message) => {
    ...
    if (explicitPlan.length) {
      upsert('plan', planEntry(`${message.id}:plan`, explicitPlan, taskProgressNote(message.agentUI)));
    }
    ...
    if (!explicitPlan.length && toolEvents.length === 0 && !message.fileEdits?.length && !messageEvidence.length) {
...
function planEntry(id: string, steps: TaskProgressStep[], note?: string): TaskNarrativeEntry {
