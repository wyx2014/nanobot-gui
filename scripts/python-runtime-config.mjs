...
export const LINUX_RUNTIME_TARGET = 'linux-x64';
...
export const RUNTIME_TARGETS = {
  ...
  'linux-x64': `cpython-${PYTHON_VERSION}+${PYTHON_STANDALONE_RELEASE}-x86_64-unknown-linux-gnu-install_only.tar.gz`,
...
export function runtimeArchiveName(target) {
  ...
  return `tpcowork-python-${PYTHON_VERSION}-${target}-${RUNTIME_PROFILE}.${extension}`;
}
