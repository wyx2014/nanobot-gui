...
Gateway 的操作事件写入工作区 `.nanobot/logs.sqlite`，`component` 以 `operations.` 开头，覆盖 HTTP、WS 请求接收、Turn、模型尝试和重试、Trace span、MCP 连接/初始化/发现/调用、PPT 渲染/校验/预览及产物发布登记。通过认证 `/api/diagnostics/logs?trace_id=...` 查询；响应中的 `collection` 提供队列深度、丢弃与写入失败计数。新增操作行按 30 天或最近 100,000 条清理，每写入 1,000 条检查一次，不改变已有 Trace、安全审计和会话数据的保留策略。
...
在 **设置 → 帮助与反馈 → 导出诊断包** 或窗口 **帮助 → 导出诊断包** 中填写问题描述、发生时间，选择 15 分钟、1 小时或 24 小时的日志范围。默认选当前会话及应用公共事件，也可明确选择该时段的所有任务。原「意见反馈」入口已移除。
...
ZIP 含 `issue.json`、环境摘要、桌面和 gateway 事件、Trace/Run/Span、安全事件、导出时运行状态、事件时间线、`summary.md`、AI 分析说明、脱敏统计和带 SHA-256 的文件清单。快照记录实际采集时间，不能代表过去的故障现场。旧文本日志只投影时间、级别、异常类型、错误代码和匿名化栈位置，正文不进入包内；原始数据库、配置、对话和文稿附件也不导出。
...
将 ZIP 和问题编号发给技术支持，即可按包内说明交给 AI 分析。自动故障快照、附件选择、远程上传和修复案例管理仍属后续范围，见 [日志与问题诊断闭环设计](docs/diagnostics-system-design.md)。
