...
describe('desktop notifications', () => {
  it('normalizes whitespace and bounds operating-system text', () => {
    ...
    expect(result?.title).toBe('TPCowork 任务完成');
  ...
  it('shows a supported notification and activates its conversation on click', () => {
    ...
    click?.();
    expect(activate).toHaveBeenCalledWith({ conversationId: 'chat-1' });
  });

  it('activates an exact schedule run on click', () => {
    let click: (() => void) | undefined;
    const native = {
      on: vi.fn((_event: 'click', listener: () => void) => {
        click = listener;
    ...
    expect(activate).toHaveBeenCalledWith({ scheduleTaskId: 'task-1', runId: 'run-7' });
  });
