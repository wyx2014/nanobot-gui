...
本规范适用于所有涉及企业财务数据的研究。A 股关键数值字段必须在当前已配置的**聚源 + 同花顺 iFinD + 财汇 Caihui MCP**之间按字段交叉核验；其他市场至少使用两个独立来源。来源误差>1%须标记。
...
| 核心来源 | 能力 | 获取方式 |
|----------|------|---------|
| **聚源金融数据 MCP** | A 股行情、财务、估值、公告与机构数据 | 当前工具列表中的 `mcp_juyuan_...` 工具 |
| **同花顺 iFinD MCP** | 公司资料、财务指标、行情、公告、新闻与行业板块 | `mcp_hexin-ifind-ds-...` 工具 |
| **财汇 Caihui MCP** | A 股公司、财务、行情、公告及宏观区域数据 | 当前工具列表中的 `mcp_caihui_mcp_...` 工具 |

三源中只核验当前已配置且覆盖该字段的来源。公司公告、交易所和监管披露用于解决口径冲突，不替代核心三源，也不得被归纳成用户全局偏好。
...
对 A 股每个财务指标（收入、净利润、毛利率、经营现金流、资产负债率等），分别从当前可用的聚源、iFinD、财汇取数；其他市场分别从**来源1**和**来源2**取数。
...
3. **原始财报优先**：若结构化来源与原始财报（10-K/年报PDF）不符，以原始财报为准，标记来源错误
...
| 三七互娱 | 聚源 + iFinD + 财汇（按可用字段核验） | 公司公告/交易所披露（冲突裁决） |
| 吉比特 | 聚源 + iFinD + 财汇（按可用字段核验） | 公司公告/交易所披露（冲突裁决） |
