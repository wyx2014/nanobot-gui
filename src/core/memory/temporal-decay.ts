...
async function extractTimestamp(params: {
  ...
  // Because browser context doesn't have path.isAbsolute, we do a basic check
  const isAbsolute = params.filePath.startsWith("/") || /^[a-zA-Z]:\\/.test(params.filePath);
  const absolutePath = isAbsolute
    ? params.filePath
    : `${params.workspaceDir}/${params.filePath}`.replace(/\/\//g, "/");

  try {
    const stat = await fsBridge.lstat(absolutePath);
