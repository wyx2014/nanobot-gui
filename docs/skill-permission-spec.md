...
| Scenario | Expected |
| --- | --- |
| Project grants `ifind-finance-data` | New and old conversations in that project may use it |
| Project removes `ifind-finance-data` | New and old conversations in that project cannot use it |
| User deletes `ifind-finance-data` | All project grants drop it; no conversation can use it |
| Old message contains `/ifind-finance-data ...` after delete | Skill is not loaded; remaining text may continue |
| Built-in skill exists | Available in all projects without project grant |
